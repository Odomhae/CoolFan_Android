...
private fun DrawScope.drawCuteBody(cx: Float, cy: Float, radius: Float, themeColors: ThemeColors) {
    drawCircle(
        brush = Brush.radialGradient(
            colors = listOf(themeColors.frame.copy(alpha = 0.6f), themeColors.frame.copy(alpha = 0.2f)),
            center = Offset(cx, cy),
            radius = radius
        ),
        radius = radius,
    ...
    drawCircle(color = themeColors.accent, radius = radius, center = Offset(cx, cy), style = Stroke(width = radius * 0.05f))
    for (i in 0 until 8) {
        val angle = (i * 45f) * Math.PI / 180.0
        drawCircle(
            color = themeColors.accent.copy(alpha = 0.6f),
            radius = radius * 0.04f,
            center = Offset(cx + radius * 0.88f * cos(angle).toFloat(), cy + radius * 0.88f * sin(angle).toFloat())
        )
    }
}
...
private fun DrawScope.drawCuteFace(cx: Float, cy: Float, hubR: Float, accentColor: Color, textColor: Color) {
    ...
    drawCircle(color = textColor, radius = eyeR, center = Offset(cx + eyeOffX, cy - eyeOffY))
    drawCircle(color = Color.White, radius = eyeR * 0.4f, center = Offset(cx - eyeOffX + eyeR * 0.2f, cy - eyeOffY - eyeR * 0.2f))
    drawCircle(color = Color.White, radius = eyeR * 0.4f, center = Offset(cx + eyeOffX + eyeR * 0.2f, cy - eyeOffY - eyeR * 0.2f))
